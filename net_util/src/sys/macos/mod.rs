...
/// Network interface trait for macOS. macOS does not have tap devices; this
/// implementation uses vmnet.framework to provide similar functionality under
/// the same interface as Linux's TapT, enabling cross-platform code sharing.
...
impl Drop for VmnetInner {
    fn drop(&mut self) {
        if !self.iface.is_null() {
            ...
            extern "C" fn stop_handler(
                block_ptr: *mut BlockWithCapture<dispatch_semaphore_t>,
                _status: u32,
            ) {
                unsafe { dispatch_semaphore_signal((*block_ptr).capture) };
            }
            let mut block = BlockWithCapture::new(stop_handler as *const std::ffi::c_void, sem);
            unsafe {
...
impl VmnetTap {
    ...
    {
        let queue =
            unsafe { dispatch_queue_create(c"org.chromium.crosvm.vmnet".as_ptr(), ptr::null()) };
        if queue.is_null() {
        ...
        let mut block = BlockWithCapture::new(start_handler as *const std::ffi::c_void, result_ptr);
        ...
        // Extract MTU
        let mtu = unsafe { xpc_dictionary_get_uint64(result.props, vmnet_mtu_key) as u16 };
        let mtu = if mtu == 0 { 1500 } else { mtu };

        // Extract max packet size
        let max_packet_size =
            unsafe { xpc_dictionary_get_uint64(result.props, vmnet_max_packet_size_key) as usize };
    ...
    fn setup_event_callback(
        ...
        let mut block =
            BlockWithCapture::new(event_handler as *const std::ffi::c_void, notify_write);
...
impl TapTCommon for VmnetTap {
    ...
    fn try_clone(&self) -> Result<Self> {
        ...
        let mut pipe_fds = [0i32; 2];
        if unsafe { libc::pipe(pipe_fds.as_mut_ptr()) } != 0 {
            return Err(Error::Vmnet(
                "failed to create notification pipe for clone".into(),
            ));
        }
        let notify_read = pipe_fds[0];
        let notify_write = pipe_fds[1];
