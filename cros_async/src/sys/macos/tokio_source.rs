...
use base::unix::clone_descriptor;
...
fn do_read(raw: Arc<OwnedFd>, file_offset: Option<u64>, buf: &mut [u8]) -> io::Result<usize> {
    ...
    let res = match file_offset {
        // SAFETY: we partially own `raw`, `ptr` has space up to buf.len()
...
fn do_write(raw: Arc<OwnedFd>, file_offset: Option<u64>, buf: &[u8]) -> io::Result<usize> {
    ...
    let res = match file_offset {
        // SAFETY: we partially own `raw`, `ptr` has data up to buf.len()
...
fn do_punch_hole(raw: Arc<OwnedFd>, file_offset: u64, len: u64) -> io::Result<()> {
    use std::fs::File;
    use std::os::fd::FromRawFd;
    use std::os::fd::IntoRawFd;

    use base::macos::file_punch_hole;

    // Temporarily wrap the fd in a File for file_punch_hole, then reclaim it.
    let fd = raw.as_raw_fd();
    // SAFETY: we partially own `raw`, and we reclaim the fd below without dropping.
    let file = unsafe { File::from_raw_fd(fd) };
    let result = file_punch_hole(&file, file_offset, len);
    // Don't let File close the fd — it's still owned by `raw`.
    let _ = file.into_raw_fd();
    result
}

fn do_write_zeroes(raw: Arc<OwnedFd>, file_offset: u64, len: u64) -> io::Result<()> {
    use std::fs::File;
    use std::os::fd::FromRawFd;
    use std::os::fd::IntoRawFd;

    use base::macos::file_write_zeroes_at;

    let fd = raw.as_raw_fd();
    // SAFETY: we partially own `raw`, and we reclaim the fd below without dropping.
    let file = unsafe { File::from_raw_fd(fd) };
    let result = file_write_zeroes_at(&file, file_offset, len as usize);
    let _ = file.into_raw_fd();
    result.map(|_| ())
}
...
impl<T: AsRawDescriptor> TokioSource<T> {
    ...
    pub async fn punch_hole(&self, file_offset: u64, len: u64) -> AsyncResult<()> {
        ...
        Ok(self
            .runtime
            .spawn_blocking(move || do_punch_hole(fd, file_offset, len))
            .await
            .map_err(Error::Join)?
    ...
    pub async fn write_zeroes_at(&self, file_offset: u64, len: u64) -> AsyncResult<()> {
        ...
        Ok(self
            .runtime
            .spawn_blocking(move || do_write_zeroes(fd, file_offset, len))
            .await
            .map_err(Error::Join)?
            .map_err(Error::Fallocate)?)
    }
