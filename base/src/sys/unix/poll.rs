...
//! POSIX poll(2)-based event multiplexing. This implementation is cross-platform
//! and can be used on any Unix system. On macOS it is used as the primary
//! EventContext; on Linux it can be used for testing since Linux normally uses
//! the epoll-based EventContext.
...
/// An EventContext backed by POSIX poll(2). This is cross-platform and can be
/// compiled on any Unix system; macOS aliases this as its primary `EventContext`.
pub struct PollEventContext<T> {
    entries: std::sync::Mutex<Vec<PollEntry>>,
    _phantom: std::marker::PhantomData<[T]>,
}

// SAFETY: The Mutex provides interior synchronization
unsafe impl<T> Send for PollEventContext<T> {}
// SAFETY: The Mutex provides interior synchronization
unsafe impl<T> Sync for PollEventContext<T> {}

impl<T: EventToken> PollEventContext<T> {
    /// Creates a new PollEventContext.
    pub fn new() -> crate::errno::Result<PollEventContext<T>> {
        Ok(PollEventContext {
    ...
    /// Creates a new PollEventContext with initial file descriptors and tokens.
    pub fn build_with(
        fd_tokens: &[(&dyn AsRawDescriptor, T)],
    ) -> crate::errno::Result<PollEventContext<T>> {
        let ctx = PollEventContext::new()?;
    ...
    pub fn add(&self, descriptor: &dyn AsRawDescriptor, token: T) -> crate::errno::Result<()> {
        self.add_for_event(descriptor, EventType::Read, token)
    }

    /// Adds a file descriptor to the PollEventContext with the specified event type.
    ...
    /// Modifies an existing file descriptor in the PollEventContext.
    pub fn modify(
        &self,
    ...
    /// Removes a file descriptor from the PollEventContext.
...
impl<T> AsRawDescriptor for PollEventContext<T> {
    fn as_raw_descriptor(&self) -> RawDescriptor {
        -1
    }
}

impl<T> Drop for PollEventContext<T> {
    fn drop(&mut self) {}
}
