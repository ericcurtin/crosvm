...
impl HvfVcpu {
    ...
    fn ensure_created(&mut self) -> Result<()> {
        ...
        if ret != bindings::HV_SUCCESS {
            base::error!(
                "hv_vcpu_create failed for vcpu {}: 0x{:08x}",
                self.id,
                ret as u32
            );
        ...
        for region in self.guest_mem.regions() {
            let flags: u64 =
                (bindings::HV_MEMORY_READ | bindings::HV_MEMORY_WRITE | bindings::HV_MEMORY_EXEC)
                    .into();
            // Unmap first (ignore errors - might not have been mapped yet from this thread's
            // perspective)
            ...
            if ret != bindings::HV_SUCCESS {
                base::error!(
                    "vcpu {}: re-map guest memory failed: 0x{:08x}",
                    self.id,
                    ret as u32
                );
            }
        ...
        base::info!(
            "vcpu {}: created on thread, applying {} cached registers",
            self.id,
            num_cached
        );
    ...
    fn handle_psci(&self, func_id: u32) -> Result<VcpuExit> {
        match func_id {
            ...
            // PSCI_CPU_ON (32-bit)
            0x84000003 => Ok(VcpuExit::SystemEventReset),
            // PSCI_CPU_ON (64-bit)
            0xC4000003 => Ok(VcpuExit::SystemEventReset),
            // PSCI_CPU_OFF
            0x84000002 => Ok(VcpuExit::Shutdown(Ok(()))),
            // PSCI_SYSTEM_OFF
            0x84000008 => Ok(VcpuExit::Shutdown(Ok(()))),
            // PSCI_SYSTEM_RESET
            0x84000009 => Ok(VcpuExit::SystemEventReset),
            // PSCI_FEATURES
            0x8400000A => {
    ...
    fn reg_to_hvf(&self, reg: VcpuRegAArch64) -> Option<HvfRegType> {
        match reg {
            ...
            VcpuRegAArch64::System(sys_id) => aarch64_sysreg_to_hvf(sys_id).map(HvfRegType::SysReg),
            _ => None,
        }
    }
}
    ...
    fn run(&mut self) -> Result<VcpuExit> {
        ...
        if self.vtimer_masked {
            ...
            if !irq_active {
                // SAFETY: vcpu is valid
                unsafe {
                    bindings::hv_vcpu_set_vtimer_mask(self.vcpu, false);
                }
            ...
            bindings::HV_EXIT_REASON_EXCEPTION => {
                ...
                match ec {
                    bindings::EC_DATAABORT_LOWER_EL => {
                        ...
                        let data = if is_write { self.get_reg(srt)? } else { 0 };
                    ...
                    bindings::EC_SYSTEMREGISTERTRAP => {
                        ...
                        let sysreg_desc = format!("S{}_{}_C{}_C{}_{}", op0, op1, crn, crm, op2);
...
impl VcpuAArch64 for HvfVcpu {
    ...
    fn get_psci_version(&self) -> Result<PsciVersion> {
        Ok(PsciVersion { major: 1, minor: 0 })
    }
