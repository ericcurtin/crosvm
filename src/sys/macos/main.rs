...
fn create_virtio_devices(
    ...
    for disk in &cfg.disks {
        ...
        let block_dev = virtio::BlockAsync::new(
            features, disk_image, disk, None, // control_tube
            None, // queue_size
            None, // num_queues
        )
        .context("failed to create block device")?;

        let (msi_host_tube, msi_device_tube) = Tube::pair().context("failed to create MSI tube")?;
...
fn spawn_vm_memory_handler_thread(
    ...
    thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let wait_ctx: WaitContext<MemToken> =
                WaitContext::new().expect("failed to create VM memory handler WaitContext");
...
pub fn run_config(cfg: Config) -> anyhow::Result<ExitState> {
    ...
    let guest_memory_layout = Arch::guest_memory_layout(&components, &arch_memory_layout, &hvf)
    ...
    let system_allocator_config = Arch::get_system_allocator_config(&vm, &arch_memory_layout);
    let mut system_allocator = resources::SystemAllocator::new(system_allocator_config, None, &[])
    ...
    let mut irq_chip = HvfIrqChip::new(vcpu_count).context("failed to create HVF IRQ chip")?;

    // Step 6.5: Create virtio devices (net, etc.)
    let (devices, irq_host_tubes, vm_memory_host_tubes) = create_virtio_devices(&cfg, &guest_mem)?;
    ...
    let wait_ctx: WaitContext<Token> = WaitContext::build_with(&[(&vm_evt_rdtube, Token::VmEvent)])
