...
fn initialize_hvf_gic(num_vcpus: usize) -> std::result::Result<[u64; 4], String> {
    ...
    if ret != hvf_gic_ffi::HV_SUCCESS {
        ...
        info!(
            "hv_gic_get_distributor_size unavailable, using default {:#x}",
            dist_size
        );
    }
    ...
    if ret != hvf_gic_ffi::HV_SUCCESS {
        ...
        info!(
            "hv_gic_get_redistributor_size unavailable, using default {:#x}",
            redist_size
        );
    }
    ...
    let ret = unsafe { hvf_gic_ffi::hv_gic_config_set_redistributor_base(config, redist_base) };
    if ret != hvf_gic_ffi::HV_SUCCESS {
        return Err(format!(
...
impl IrqChip for HvfIrqChip {
    ...
    fn register_edge_irq_event(
        ...
        info!(
            "register_edge_irq_event: gsi={} index={} device={}",
            irq, index, source.device_name
        );
    ...
    fn irq_event_tokens(&self) -> Result<Vec<(IrqEventIndex, IrqEventSource, Event)>> {
        ...
        for (index, evt) in events.iter().enumerate() {
            tokens.push((index, evt.source.clone(), evt.trigger_event.try_clone()?));
        }
        Ok(tokens)
    }
    ...
    fn service_irq_event(&mut self, event_index: IrqEventIndex) -> Result<()> {
        ...
        if let Some(evt) = events.get(event_index) {
            ...
            if self.gic_initialized {
                info!(
                    "service_irq_event: index={} gsi={} device={}",
                    event_index, evt.gsi, evt.source.device_name
                );
                hvf_gic_set_spi(evt.gsi, true);
                hvf_gic_set_spi(evt.gsi, false);
            } else {
                error!(
                    "service_irq_event: GIC not initialized, dropping gsi={}",
                    evt.gsi
                );
            }
